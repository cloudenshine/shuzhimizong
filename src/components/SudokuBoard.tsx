...
const SudokuBoard: React.FC<Props> = ({
  ...
  function handleInput(row: number, col: number, value: string) {
    ...
    if (isBoardComplete(newBoard) && checkSudoku(newBoard)) onComplete();
  }

  return (
    <div style={{display:"grid", gridTemplateRows:"repeat(9,1fr)", border:"2px solid #444"}}>
          ...
          {rowArr.map((cell, cIdx) => (
            <input
              ...
              style={{
                ...
                border: (rIdx%3===2&&rIdx!==8?"2px solid #999":"1px solid #ccc") + " " + (cIdx%3===2&&cIdx!==8?"2px solid #999":"1px solid #ccc"),
                background: initialBoard[rIdx][cIdx]===0 ? "#fff" : "#f7f9fb",
                fontWeight: initialBoard[rIdx][cIdx]===0 ? "bold" : "normal"
              }}
              type="text"
              maxLength={1}
              value={cell===0 ? "" : cell}
              onChange={e=>handleInput(rIdx, cIdx, e.target.value)}
              disabled={initialBoard[rIdx][cIdx]!==0||completed}
            />
          ))}
        </div>
      )}
